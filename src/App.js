  ...
  const init = async (categoryList) => {
    ...
    await axios.all(
      categoryList?.map(async (item) => {
        ...
        let tempValue = !isToday
          ? dateWiseFilter(response?.data)
          : response?.data;
    ...
    setLoading(false);
    setNewsData(tempAll);
    globalData?.randomList?.length === 0 &&
      setGlobalData({ ...globalData, randomList: tempAll });
  };

  const dateWiseFilter = (list = []) => {
    let temp = list.filter(
      (item) => moment().format("DD MMM YYYY,dddd") === item.date
    );
